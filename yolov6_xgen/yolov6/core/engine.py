...
class Trainer:
    ...
    def train_in_loop(self, epoch_num):
        try:
            ...
            CL.after_scheduler_step(epoch=epoch_num)
            # Cocopie end
        except Exception as _:
            print(_)
    ...
    def eval_and_save(self):
        ...
        if self.main_process:
            ...
            ckpt_state_dict = deepcopy(de_parallel(self.model)).state_dict()
            save_ckpt_dir = osp.join(self.save_dir, 'weights')
            save_checkpoint(ckpt, (is_val_epoch) and (self.ap == self.best_ap), save_ckpt_dir, model_name='last_ckpt')
            from yolov6.utils.export_onnx import export_onnx

            model_dummy = self.ema.ema if self.ema else self.model
            xgen_record(self.args_ai, model_dummy, float(self.ap), epoch=self.epoch)
            export_onnx(self.args_ai, copy.deepcopy(model_dummy))
