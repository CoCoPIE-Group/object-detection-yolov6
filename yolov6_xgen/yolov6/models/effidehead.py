...
class Detect(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            obj = torch.ones((pred_bboxes.shape[0], pred_bboxes.shape[1], 1), device=pred_bboxes.device, dtype=pred_bboxes.dtype)
            # tmp = torch.cat([pred_bboxes.unsqueeze(0), obj.unsqueeze(0)], dim=-1)
            # result = torch.cat([tmp, cls_score_list.unsqueeze(0)], dim=-1)
